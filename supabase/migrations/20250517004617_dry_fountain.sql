/*
  ...
  1. New Tables
    ...
    - `wallets` - Stores wallet balances for users
  
  2. Views
    - `driver_wallets` - View for driver wallet information
    - `vendor_wallets` - View for vendor wallet information
  
  3. Functions
    - Functions for wallet management (charging, getting balances, etc.)
...
SELECT 
  ...
  d.rating_count,
  d.commission_rate,
  (SELECT COUNT(*) FROM orders WHERE driver_id = d.id) AS total_orders,
  (SELECT COUNT(*) FROM orders WHERE driver_id = d.id AND status = 'completed') AS completed_orders,
  (SELECT COALESCE(SUM(total), 0) FROM orders WHERE driver_id = d.id) AS total_earnings
FROM 
  wallets w
...
SELECT 
  ...
  (SELECT COALESCE(SUM(total), 0) FROM orders WHERE vendor_id = v.id) AS total_earnings
FROM 
  wallets w
