...
-- Create function to update updated_at if it doesn't exist already
DO $$
DECLARE
  function_exists BOOLEAN;
BEGIN
  SELECT EXISTS(SELECT FROM pg_proc WHERE proname = 'update_vendor_categories_table_updated_at') INTO function_exists;
  
  IF NOT function_exists THEN
    EXECUTE '
    CREATE FUNCTION update_vendor_categories_table_updated_at()
    RETURNS TRIGGER AS $$
    BEGIN
      NEW.updated_at = now();
      RETURN NEW;
...
BEGIN
  SELECT EXISTS(
    ...
    AND tgrelid = 'vendor_categories_table'::regclass
  ) INTO trigger_exists;
